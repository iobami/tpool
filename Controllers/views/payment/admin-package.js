...
exports.create = async (req, res) => {
  try {
    ...
    const { 
      package_name, price, description, features 
    } = req.body;
    ...
    if (query) {
      ...
      const result = await Package.create({ 
        package_name, price, description, duration, package_id 
      });
      ...
      if (features.length = 0) {
        ...
        req.flash('error', 'Kindly select a supported Feature for this Package')
        return res.redirect('/admin/packages/create')
...
exports.packageUpdate = async (req, res) => {
  try {
    const { 
      package_name, price, description, duration, features 
    } = req.body;
